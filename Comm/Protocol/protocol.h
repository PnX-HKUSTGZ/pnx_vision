...
private:
  static uint16_t get_protocol_info_vision(uint8_t *rx_buf,
   ...
   * @param flags_register a 2-byte register to store some flag, usually
   * shoot/mode etc.
   * @param tx_data data ready to transmit, always float type
   * @param float_length length of tx_data
   * @param tx_buf start position of tx_buf, which is to store all bytes for
   * send
   * @param tx_buf_len bytes, indicate the length of tx_buf
   */
  static void
  get_protocol_send_data_vision(uint16_t send_id, uint16_t flags_register,
                                float *tx_data, uint8_t float_length,
