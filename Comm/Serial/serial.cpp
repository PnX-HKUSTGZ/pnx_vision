...
void Serial::clear_input_buffer() const { tcflush(serial_port, TCIFLUSH); }

void Serial::clear_output_buffer() const { tcflush(serial_port, TCOFLUSH); }

void Serial::clear_input_output_buffer() const {
  tcflush(serial_port, TCIOFLUSH);
}

Serial::~Serial() { close_port(); }
...
void Serial::recv(std::string &s) { s = try_recv_for(s, 0); }

bool Serial::rm_init() {
  tty.c_cflag &= ~PARENB; // Clear parity bit, disableing parity
  tty.c_cflag &= ~CSTOPB; // Clear stop field, only use one stop bit, Otherwise
                          // two bits are used
  tty.c_cflag &=
  ...
  tty.c_cflag &= ~CRTSCTS; // Disable RTS/CTS hardware flow control to avoid
                           // infinite waiting, RTS - request transmit
  tty.c_cflag |=
  ...
  tty.c_oflag &= ~OPOST; // Prevent special interpretation of output bytes (eg:
                         // newline chars)
  tty.c_oflag &=
      ~ONLCR; // Prevent conversion of newline to carriage return / line feed

  // READ SETTINGS: Only work at block mode
  tty.c_cc[VTIME] = 1; // wait for up to 100ms between characters since first
