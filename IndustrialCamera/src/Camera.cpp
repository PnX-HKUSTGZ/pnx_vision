...
namespace hnurm
{
    HKcam::HKcam(const cv::FileNode &cfg_node)
    {
        ...
        {
            CLOG(INFO, "camera") << "Camera " << _id << " opened successfully.";
        } else
        {
            CLOG(ERROR, "camera") << "Camera " << _id << " failed to open!";
            exit(-1);
        }
    }
    ...
    bool HKcam::GetFrame(ImgInfo &img)
    {
        nRet = MV_OK;
        ...
        auto tmp_bayer = cv::Mat(stImageInfo.nHeight, stImageInfo.nWidth, CV_8UC1, pData);
        cvtColor(tmp_bayer, img.img, cv::COLOR_BayerGB2RGB);
        ...
        switch (_nImageOrientation)
        {
            case 0:
                break;
            case 1:
                cv::rotate(img.img, img.img, cv::ROTATE_90_CLOCKWISE);
                break;
            case 2:
                cv::rotate(img.img, img.img, cv::ROTATE_180);
                break;
            case 3:
                cv::rotate(img.img, img.img, cv::ROTATE_90_COUNTERCLOCKWISE);
                break;
        }
        return true;
    }
