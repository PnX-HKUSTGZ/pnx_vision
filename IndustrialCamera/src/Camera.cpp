...
HKcam::HKcam() {
  ...
  _id = "DA1195491";
  _cfg_path = "/home/neo/pnx/CameraParam.yaml";
  _nImageOrientation = 0;

  if (OpenCam(_id)) // here open and config camera
  {
    cout << "Camera " << _id << " opened successfully.";
  } else {
    cout << "Camera " << _id << " failed to open!";
    exit(-1);
  }
}

HKcam::~HKcam() { CloseCam(); }
...
bool HKcam::GetFrame(cv::Mat &img) {
  nRet = MV_OK;
  ...
  // cvtColor(tmp_bayer, img, cv::COLOR_RGB2BGR);
...
string HKcam::GetCamName() { return _id; }
